...
class NavListPublishSpec extends ComponentSpec {
    ...
    def "Nav List: Stacked with Children List has correct current page marks"() {
        ...
        when: "I am on the component showcase page"
        setWindowSize(viewport)
        waitForAuthorPreviewPageUrl("content/${clientName}-core-showcase/en/component/lists/nav-list/pages/page2/page2.html")
    ...
    def "Nav List: Dropdown with Children List"() {
        ...
        assert ${symbol_dollar}("[aria-labelledby=navlist6_page1] > a.nav-link").size() == 5

        and: "First item has five items"
        assert ${symbol_dollar}("[aria-labelledby=navlist6_page1_page1] > a.nav-link").size() == 2
