...
//        assert ${symbol_dollar}(selector).attr("class").contains("bg-c8")
    ...
    def "Functionality of ${clientName}-core Global"() {
        ...
        and: '>I am in the component showcase page'
        and: '>the component is on the showcase page'
        def selector = "${symbol_pound}footer_${clientName}-core_global"
        ...
        assert ${symbol_dollar}(selector + " ${symbol_pound}embedsource_AP3MMJNLO > img").attr("src").contains("/etc/clientlibs/${clientName}-core/sut/images/${clientName}-core-logo.svg")
